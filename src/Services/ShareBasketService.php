...
readonly class ShareBasketService implements ShareBasketServiceInterface
{
    ...
    public function prepareLineItems(SalesChannelContext $salesChannelContext): array
    {
        ...
        usort($lineItems, static fn (array $a, array $b): int => strcmp($a['identifier'], $b['identifier']));
    ...
    public function cleanup(): ?EntityWrittenContainerEvent
    {
        ...
        $shareBasketEntities = $this->froshShareBasketRepository->searchIds($criteria, $context);

        if (($ids = $shareBasketEntities->getIds()) === []) {
            return null;
        }

        $ids = array_map(static fn ($id) => ['id' => $id], $ids);

        return $this->froshShareBasketRepository->delete($ids, $context);
    }
    ...
    private function addLineItems(
        ...
        foreach ($shareBasketEntity->getLineItems() as $shareBasketLineItemEntity) {
            try {
                $this->eventDispatcher->dispatch(
                    $event = new ShareBasketAddLineItemEvent($cart, $salesChannelContext, $shareBasketLineItemEntity),
                    ShareBasketAddLineItemEvent::class,
                );

                if ($event->getLineItem() !== null) {
                    $lineItems[] = $event->getLineItem();
                }

                if ($shareBasketLineItemEntity->getType() === LineItem::PRODUCT_LINE_ITEM_TYPE) {
                    $product = $this->addProduct($salesChannelContext, $shareBasketLineItemEntity);
                ...
                if ($shareBasketLineItemEntity->getType() === PromotionProcessor::LINE_ITEM_TYPE) {
                    $lineItems[] = $this->addPromotion($shareBasketLineItemEntity);
                }
            } catch (\Exception) {
            }
        }
    ...
    private function persistCart(Request $request, SalesChannelContext $salesChannelContext, array $data, int $attempts = 0): ?string
    {
        ...
        try {
            $id = Uuid::randomHex();
            $data['id'] = $id;
            $result = $this->froshShareBasketRepository->create([$data], $salesChannelContext->getContext());
